<?php
	...
	if(($hasOr !== false) && ($hasAnd !==false)){
		...
		else $docID = array_values($docID[$word]);
	}
	//var_dump($docID[0]);
	if(($hasOr !== false) && ($hasAnd !==false)){
		for($i=0; $i<2; $i++){
			foreach($inputQuery[$i] as $word){
				for($j=0; $j<count($docID); $j++){
					$query = "select docID, count, title, author, source from invertIndex where word=\"$word\"
								and docID = $docID[$j]";
					if ($result = $mysqli->query($query)){
						while ($row = $result->fetch_assoc()){
							$docID = $row["docID"];
							...
							if (!isset($titleMap[$docID])){
								...
								$authorMap[$docID] = $author;  //save title
								$sourceMap[$docID] = $source;  //save title
							}
	...
	//echo $query;
		...
		//var_dump($docID);
		$docID1 = $docID;  //very care !!!
		foreach($inputQuery as $word){
			for($j=0; $j < count($docID1); $j++){
				$query = "select docID, count, title, author, source from invertIndex where word=\"$word\"
									and docID = $docID1[$j]";
				if ($result = $mysqli->query($query)){
					while ($row = $result->fetch_assoc()){
	...
	$i = 1;
	echo "<table border=1>";
	echo "<tr><th>index</th><th>title</th><th>Num of Matches</th><th>author</th><th>source</th>";
	foreach($score as $docID=>$singleScore){
		echo "<tr>";
	    echo "<td>".$i."</td>
	    	  <td><a href=\"./docRender.php?docID=$docID\">$titleMap[$docID]</td>
	    	  <td>$singleScore</td><td>$authorMap[$docID]</td><td>$sourceMap[$docID]</td>";
