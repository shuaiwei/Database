...
if (strpos($content,'<A>') !== false){
	$authorStart = strpos($content,'<A>')+3;
	$authorEnd = strpos($content,'</A>')-2;
	$authorLength = $authorEnd - $authorStart;
	$author = substr($content,$authorStart,$authorLength);
}

if (strpos($content,'<S>') !== false){
	$sourceStart = strpos($content,'<S>')+3;
	$sourceEnd = strpos($content,'</S>')-2;
	$sourceLength = $sourceEnd - $sourceStart;
	$source = substr($content,$sourceStart,$sourceLength);
}
...
if (isset($author)){
	$author1 = preg_replace($pattern, ' ', $author);
	$author1 = strtolower($author1);
	$author1 = explode(' ',$author1);
	array_pop($author1);

	$words = array_merge($words, $author1);
}
...
foreach($wordsArray as $word=>$count){
	$indexID = $keyUnique.$i;
	$query = "insert into invertIndex(indexID,word,docID,count,title,author,source) values($indexID,\"$word\",\"$keyUnique\",$count,\"$title\",\"$author\",\"$source\");";
